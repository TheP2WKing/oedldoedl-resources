...
public class ModItemEffectNuggetBase extends ModItemBase
{
	 ...
	 */
    public ModItemEffectNuggetBase(Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, boolean isDebuff, TipLines lines, boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, ResourcesConfig.nugget_base_duration.get(), ResourcesConfig.nugget_base_amplifier.get() - 1, false, false));
		}
	}
	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) 
    {
		...
		if (Screen.hasControlDown() && CoreConfig.effects_tooltip.get() && ResourcesConfig.nugget_effects_enabled.get() && ResourcesConfig.effects_enabled.get())
		{
			...
			tooltip.add((new StringTextComponent("- ").mergeStyle(CoreConfig.effects_formatting.get()).mergeStyle(TextFormatting.ITALIC).appendSibling(new TranslationTextComponent(effect.getName())
					...
					.appendSibling(new StringTextComponent(" " + effectColor() + TextFormatting.ITALIC + "("+ ResourcesConfig.nugget_base_amplifier.get() + ") (" 
