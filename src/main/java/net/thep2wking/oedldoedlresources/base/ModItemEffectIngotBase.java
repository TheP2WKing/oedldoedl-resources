...
public class ModItemEffectIngotBase extends ModItemBase
{
	 ...
	 */
    public ModItemEffectIngotBase(Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, boolean isDebuff, TipLines lines, boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, ResourcesConfig.ingot_base_duration.get(), ResourcesConfig.ingot_base_amplifier.get() - 1, false, false));
		}
	}
    ...
    {
		if (Screen.hasShiftDown() && CoreConfig.information_tooltip.get() && !(lines == TipLines.DISABLED))
		{   
			if(hasAnnotation)
			{
		...
		if (Screen.hasControlDown() && CoreConfig.effects_tooltip.get() && ResourcesConfig.ingot_effects_enabled.get() && ResourcesConfig.effects_enabled.get())
		{
			...
			tooltip.add((new StringTextComponent("- ").mergeStyle(CoreConfig.effects_formatting.get()).mergeStyle(TextFormatting.ITALIC).appendSibling(new TranslationTextComponent(effect.getName())
					...
					.appendSibling(new StringTextComponent(" " + effectColor() + TextFormatting.ITALIC + "("+ ResourcesConfig.ingot_base_amplifier.get() + ") (" 
