...
public class BedrockiumBlockItem extends ModBlockItemEffectBase
{
    public Effect effect2;

	public BedrockiumBlockItem(Block blockIn, Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, Effect effect2, boolean isDebuff, TipLines lines, 
			boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, 10, ResourcesConfig.block_base_amplifier.get() - 1 + 4, false, false));
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect2, ResourcesConfig.block_base_amplifier.get(), ResourcesConfig.block_base_amplifier.get() - 1 + 2, false, false));
		}
	}
	
	public int getEffect()
	{
		return ResourcesConfig.block_base_amplifier.get() + 4;
	}
	...
	{
		return ResourcesConfig.block_walking_base_amplifier.get() + 4;
	}
