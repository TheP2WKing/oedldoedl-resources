...
{
	public BlockChaosPlank(SoundType sound, int harvestLevel, float hardness, float resistance, ToolType toolType, int lightLevel, Properties properties) 
	{
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) 
	{
		if (ResourcesConfig.enable_chaos_plank_sound.get())
		{
			if (!worldIn.isRemote)
			{
				...
				{
					worldIn.playSound(null, pos, ModSounds.APPLAUSE.get(), SoundCategory.BLOCKS, 1f, 1f);
				}
			}
