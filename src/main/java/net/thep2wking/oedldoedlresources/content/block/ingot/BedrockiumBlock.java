...
public class BedrockiumBlock extends ModBlockEffectBase implements TOPInfoProvider
{
	public Effect effect2;
	
	public BedrockiumBlock(SoundType sound, int harvestLevel, float hardness, float resistance, ToolType toolType, Effect effect, Effect effect2, boolean isDebuff, int lightLevel, Properties properties) 
	{
	...
	public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, 10, ResourcesConfig.block_walking_base_amplifier.get() - 1 + 5, false, false));
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect2, ResourcesConfig.block_walking_base_duration.get(), ResourcesConfig.block_walking_base_amplifier.get() - 1 + 2, false, false));
		}
	}
