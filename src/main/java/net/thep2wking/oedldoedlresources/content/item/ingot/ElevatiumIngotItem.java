...
public class ElevatiumIngotItem extends ModItemEffectIngotBase
{
    public ElevatiumIngotItem(Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, boolean isDebuff, TipLines lines, boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, 10, ResourcesConfig.ingot_base_amplifier.get() - 1 + 9, false, false));
		}
	}
	
	public int getEffect()
	{
		return ResourcesConfig.ingot_base_amplifier.get() + 9;
	}
