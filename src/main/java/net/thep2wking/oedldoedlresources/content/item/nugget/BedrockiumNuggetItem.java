...
public class BedrockiumNuggetItem extends ModItemEffectNuggetBase
{
    public Effect effect2;

	public BedrockiumNuggetItem(Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, Effect effect2,boolean isDebuff, TipLines lines, boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		if (ResourcesConfig.nugget_effects_enabled.get() && ResourcesConfig.effects_enabled.get())
		{
			...
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect2, ResourcesConfig.nugget_base_duration.get(), ResourcesConfig.nugget_base_amplifier.get() - 1 + 1, false, false));
		}
	}
