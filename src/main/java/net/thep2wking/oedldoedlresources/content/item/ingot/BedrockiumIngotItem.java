...
public class BedrockiumIngotItem extends ModItemEffectIngotBase
{
    public Effect effect2;

	public BedrockiumIngotItem(Rarity rarity, boolean hasEffect, boolean fireImmunity, Effect effect, Effect effect2, boolean isDebuff, TipLines lines, boolean hasAnnotation, Properties properties) 
    {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int ambient, boolean particles) 
	{
		...
		{
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect, 10, ResourcesConfig.ingot_base_amplifier.get() - 1 + 4, false, false));
			((LivingEntity) entityIn).addPotionEffect(new EffectInstance(effect2, ResourcesConfig.ingot_base_duration.get(), ResourcesConfig.ingot_base_amplifier.get() - 1 + 2, false, false));
		}
	}
	
	public int getEffect()
	{
		return ResourcesConfig.ingot_base_amplifier.get() + 4;
	}
